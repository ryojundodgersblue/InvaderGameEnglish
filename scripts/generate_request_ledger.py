...
HANDLED = {
    ...
    137: ('対応済 ✔', f'フリーズ検知を30秒→20秒に短縮し定数化({C_FREEZE})。タイマー進行中は'
                     f'誤検知しないよう調整(d1523eb)。実機で20秒超の回答待ち→誤表示なしを確認'),
    ...
    139: ('対応済 ✔', f'認証切れを検知したら自動でログイン画面へ移動し「セッションの有効期限が切れました。'
                     f'ログインし直してください。」を表示({C_AUTH}/12a5162)。実機で誘導動作を確認'),
    140: ('対応済 ✔', f'No139と同対応。スタート押下時の認証切れも自動でログイン画面へ誘導({C_AUTH}/12a5162)'),
