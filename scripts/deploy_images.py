...
入力(読み取りのみ・変更しない):
    <IMAGE_SRC_ROOT>/修正画像/   差し替え画像(②付きは最新版)
    <IMAGE_SRC_ROOT>/仮/         新規追加・再修正画像

    IMAGE_SRC_ROOT は環境変数で指定。未指定時はリポジトリの親ディレクトリの
    「英会話ゲーム画像」(例: ~/Documents/英会話ゲーム/英会話ゲーム画像) を使う。
...
SRC_ROOT = os.environ.get(
    'IMAGE_SRC_ROOT',
    os.path.join(os.path.dirname(ROOT), '英会話ゲーム画像'),
)
SRC_DIRS = [
    ('修正画像', os.path.join(SRC_ROOT, '修正画像')),
    ('仮', os.path.join(SRC_ROOT, '仮')),
]
...
def main():
    ...
    with open(REPORT, 'w', encoding='utf-8') as f:
        f.write('# 画像反映マニフェスト\n\n')
        f.write(f'- 反映元: `{SRC_ROOT}` 配下の `修正画像`, `仮`\n')
