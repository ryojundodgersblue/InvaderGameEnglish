...
const logInfo = (id, msg, extra) =>
  console.info(
    `[${now()}] [${NS}] [${id}] INFO  ${msg}${extra ? " " + JSON.stringify(extra) : ""}`,
  );
const logWarn = (id, msg, extra) =>
  console.warn(
    `[${now()}] [${NS}] [${id}] WARN  ${msg}${extra ? " " + JSON.stringify(extra) : ""}`,
  );
const logError = (id, msg, extra) =>
  console.error(
    `[${now()}] [${NS}] [${id}] ERROR ${msg}${extra ? " " + JSON.stringify(extra) : ""}`,
  );
...
router.post(
  ...
  async (req, res) => {
    ...
    logInfo(reqId, "request received", {
      ip: req.ip,
    ...
    if (!SPREADSHEET_ID) {
      ...
      return res
        .status(500)
        .json({ ok: false, message: "SHEET_ID が未設定です" });
    }

    try {
      // Redisキャッシュを確認
      const cacheKey = getSheetsKey(USER_SHEET_NAME, "A1:K");
      let rows = await getCache(cacheKey);

      if (rows) {
        logInfo(reqId, "users fetched from cache", { totalRows: rows.length });
      } else {
        ...
        logInfo(reqId, "users fetched from API", { totalRows: rows.length });
      }
      ...
      if (!headerOk) {
        logError(reqId, "header mismatch", {
          got: header,
        ...
        return res
          ...
          .json({
            ok: false,
            message: "usersヘッダが想定と異なります（列名・順序固定）",
          });
      }
      ...
      const rowIndex = dataRows.findIndex(
        (r) => String(r[COL.user_id] || "") === String(userId),
      );
      if (rowIndex === -1) {
      ...
      if (isPasswordHashed(storedPassword)) {
        ...
        try {
          ...
          logError(reqId, "password verification error", {
            message: err?.message,
          });
          return res
            .status(500)
        ...
        logWarn(reqId, "plain-text password detected", {
          userId: maskUser(userId),
        });
      }
      ...
      // 管理者フラグ（固定列 index=8）
      const is_admin =
        row[COL.is_admin] === true ||
        String(row[COL.is_admin] || "").toLowerCase() === "true";
